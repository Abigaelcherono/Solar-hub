...
function renderProducts(filter = "all") {
    ...
    const qtyOptions = Array.from({ length: 10 }, (_, i) => `<option value="${i + 1}">${i + 1} unit${i > 0 ? "s" : ""}</option>`).join("");
...
function updateCartUI() {
    ...
    const itemsContainer = getEl("cartItems");
    const emptyMsg = getEl("cartEmpty");
    const footer = getEl("cartFooter");
...
function enquirePackage(packageName) {
    ...
    setTimeout(() => {
        const select = getEl("cinterest");
        select.value = "Complete Home System";
        const msg = getEl("cmessage");
...
function setupCartEvents() {
    ...
    getEl("checkoutBtn").addEventListener("click", () => {
        ...
        const itemsList = cart.map((item) => `• ${item.quantity}× ${item.product.name} @ ${formatPrice(item.product.price)} each`).join("%0A"); // %0A is URL-encoded newline
        const total = cart.reduce((sum, item) => sum + item.product.price * item.quantity, 0);
        const message = `Hello SolarHub!%0A%0AI'd like to order:%0A${itemsList}%0A%0ATotal: ${formatPrice(total)}%0A%0APlease confirm availability and delivery.`;
...
function setupContactForm() {
    ...
    form.addEventListener("submit", (e) => {
        ...
        const data = {
            name: (getEl("cname")).value.trim(),
            phone: (getEl("cphone")).value.trim(),
            email: (getEl("cemail")).value.trim(),
            interest: (getEl("cinterest")).value,
            message: (getEl("cmessage")).value.trim(),
        };
        // Basic validation
